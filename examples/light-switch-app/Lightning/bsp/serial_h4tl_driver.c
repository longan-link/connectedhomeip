...
void hci_uart_write(uint8_t * bufptr, uint32_t size, void (*callback)(void *, uint8_t), void * dummy)
{
    ...
    if ((Patch_params->patch_flag2 & (1 << (PATCH_FUNC_HCI_UART_WRITE - 32))) &&
        (Patch_params->patch_func[PATCH_FUNC_HCI_UART_WRITE]))
...
#ifdef PATCH_FUNC_UART0_IRQHandler
    if ((Patch_params->patch_flag & (1 << (PATCH_FUNC_UART0_IRQHandler))) &&
        (Patch_params->patch_func[PATCH_FUNC_UART0_IRQHandler]))
