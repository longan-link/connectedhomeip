...
static uint8_t mac_addr[6]         = { LN_BLE_MAC_ADDR0, LN_BLE_MAC_ADDR1, LN_BLE_MAC_ADDR2,
                                       LN_BLE_MAC_ADDR3, LN_BLE_MAC_ADDR4, LN_BLE_MAC_ADDR5 };
static uint8_t mac_addr_reverse[6] = { LN_BLE_MAC_ADDR5, LN_BLE_MAC_ADDR4, LN_BLE_MAC_ADDR3,
                                       LN_BLE_MAC_ADDR2, LN_BLE_MAC_ADDR1, LN_BLE_MAC_ADDR0 };
...
void ln_load_config_info(void)
{
    ...
    else
    {
        ...
        if (mac_addr[0] == LN_BLE_MAC_ADDR0 && mac_addr[1] == LN_BLE_MAC_ADDR1 && mac_addr[2] == LN_BLE_MAC_ADDR2 &&
            mac_addr[3] == LN_BLE_MAC_ADDR3 && mac_addr[4] == LN_BLE_MAC_ADDR4 && mac_addr[5] == LN_BLE_MAC_ADDR5)
        {
...
void creat_usr_app_task(void)
{
    ...
    if (OS_OK !=
        OS_ThreadCreate(&g_nim_app_thread, "nimApp", nim_app_task_entry, NULL, OS_PRIORITY_NORMAL, NIM_APP_TASK_STACK_SIZE))
    {
        LN_ASSERT(1);
    }
    /* print sdk version */
    {
        LOG(LOG_LVL_INFO, "LN882H SDK Ver: %s [build time:%s][0x%08x]\r\n", LN882H_SDK_VERSION_STRING, LN882H_SDK_BUILD_DATE_TIME,
            LN882H_SDK_VERSION);
    }
}
