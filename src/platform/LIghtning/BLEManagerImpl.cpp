 ...
 *          for the Lightning (NimBLE) platform.
 */
...
#include <platform/Lightning/BLEManagerImpl.h>
...
#define CONFIG_ENABLE_lightning_BLE_CONTROLLER 1
...
struct LightningChipServiceData
{
...
#if CONFIG_ENABLE_lightning_BLE_CONTROLLER
...
const ble_uuid128_t UUID_CHIPoBLEChar_Additional = {
...
#if CHIP_ENABLE_ADDITIONAL_DATA_ADVERTISING
              {
                  .uuid       = (ble_uuid_t *) (&UUID_CHIPoBLEChar_Additional),
...
void BLEManagerImpl::HandleConnectFailed(CHIP_ERROR error)
{
    if (sInstance.mIsCentral)
    {
        ...
        event.Type                                    = DeviceEventType::kPlatformLightningBLECentralConnectFailed;
...
CHIP_ERROR BLEManagerImpl::_Init()
{
    ...
    {
        LOG(LOG_LVL_INFO, "WiFi station already provisioned, not initializing BLE");

        //esp_bt_mem_release(ESP_BT_MODE_BTDM);
        //TODO:
        //release bt mem here to save memory
...
void BLEManagerImpl::HandlePlatformSpecificBLEEvent(const ChipDeviceEvent * apEvent)
{
    ...
    // switch (apEvent->Type)
    // {
    // case DeviceEventType::kLightningBLEConnected:
    ...
    // case DeviceEventType::kPlatformLightningBLECentralConnectFailed:
    //     if (BLEManagerImpl::mBLEScanConfig.mBleScanState == BleScanState::kConnecting)
    //     {
...
bool BLEManagerImpl::CloseConnection(BLE_CONNECTION_OBJECT conId)
{
    ...
    // Signal the LN BLE layer to close the conneecction.
                                    ...
                                    chip::System::PacketBufferHandle data)
{
    ...
    LOG(LOG_LVL_INFO, "Sending indication for CHIPoBLE TX characteristic (con %u, len %u)", conId, data->DataLength());
...
void BLEManagerImpl::DriveBLEState(void)
{
    ...
    // Initializes the LN BLE layer if needed.
    if (mServiceMode == ConnectivityManager::kCHIPoBLEServiceMode_Enabled && !mFlags.Has(Flags::kLNBLELayerInitialized))
    {
        err = InitLightningBleLayer();
        SuccessOrExit(err);
    ...
    )
    {
        ...
        // LN BLE layer needs to be refreshed.
        if (!mFlags.Has(Flags::kAdvertising) || mFlags.Has(Flags::kAdvertisingRefreshNeeded))
        {
            ...
            LOG(LOG_LVL_INFO,  "NimBLE start advertising...");
...
CHIP_ERROR BLEManagerImpl::bleprph_set_random_addr(void)
{
    ...
    if (rc != 0)
    {
        LOG(LOG_LVL_ERROR, "Failed to generate random address err: %d", rc);
    ...
    if (rc != 0)
    {
        LOG(LOG_LVL_ERROR, "Failed to set random address err: %d", rc);
        return CHIP_ERROR_INTERNAL;
    }
    ...
    if (rc != 0)
    {
        LOG(LOG_LVL_ERROR, "Failed to configure random address err: %d", rc);
        return CHIP_ERROR_INTERNAL;
    }
    return CHIP_NO_ERROR;
}
...
*/   
    ...
    ln_adv_data_config();
    ln_adv_start();
    LOG(LOG_LVL_INFO, "BLE host-controller synced");
...
void BLEManagerImpl::bleprph_host_task(void * param)
{
    ...
    LOG(LOG_LVL_INFO, "BLE Host Task Started %x\r\n");
    /* This function will return only when nimble_port_stop() is executed */
    nimble_port_run();
    nimble_port_freertos_deinit();
}

CHIP_ERROR BLEManagerImpl::InitLightningBleLayer(void)
{
    CHIP_ERROR err = CHIP_NO_ERROR;

    VerifyOrExit(!mFlags.Has(Flags::kLNBLELayerInitialized), /* */);

    semaphoreHandle = xSemaphoreCreateBinary();
    if (semaphoreHandle == NULL)
    {
        err = CHIP_ERROR_NO_MEMORY;
        LOG(LOG_LVL_ERROR, "Failed to create semaphore");
        ExitNow();
    }
    ...
    sInstance.mFlags.Set(Flags::kLNBLELayerInitialized);
...
CHIP_ERROR BLEManagerImpl::ConfigureAdvertisingData(void)
{
    ...
    advData[index++] = kServiceDataTypeSize + sizeof(LightningChipServiceData);                 // length
...
void BLEManagerImpl::HandleRXCharWrite(struct ble_gatt_char_context * param)
{
    ...
    LOG(LOG_LVL_INFO, "Write request received for CHIPoBLE RX characteristic con %u %u", param->conn_handle, param->attr_handle);
...
void BLEManagerImpl::OnGattDiscComplete(const struct peer * peer, int status, void * arg)
{
    ...
    ChipDeviceEvent event;
    event.Type                                     = DeviceEventType::kLightningBLEConnected;
...
int BLEManagerImpl::ble_svr_gap_event(struct ble_gap_event * event, void * arg)
{
    ...
    case BLE_GAP_EVENT_ADV_COMPLETE:
        LOG(LOG_LVL_INFO, "BLE_GAP_EVENT_ADV_COMPLETE event");
        break;
    ...
    case BLE_GAP_EVENT_MTU:
        LOG(LOG_LVL_INFO, "BLE_GAP_EVENT_MTU = %d channel id = %d", event->mtu.value, event->mtu.channel_id);
        break;

#if CONFIG_ENABLE_lightning_BLE_CONTROLLER
    case BLE_GAP_EVENT_NOTIFY_RX:
        LOG(LOG_LVL_INFO, "BLE_GAP_EVENT_NOTIFY_RX received %s conn_handle:%d attr_handle:%d attr_len:%d",
